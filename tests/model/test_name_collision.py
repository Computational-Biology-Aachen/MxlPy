...
def test_add_variable_then_parameter_same_name() -> None:
    model = KineticModelBuilder().add_variable("x", 1.0)
    with pytest.raises(NameError):
        model.add_parameter("x", 2.0)


def test_add_parameter_then_variable_same_name() -> None:
    model = KineticModelBuilder().add_parameter("x", 1.0)
    with pytest.raises(NameError):
...
def test_add_parameter_then_reaction_same_name() -> None:
    model = KineticModelBuilder().add_parameter("v1", 1.0).add_variable("S", 1.0)
    with pytest.raises(NameError):
        model.add_reaction(
            "v1",
...
def test_add_variable_then_reaction_same_name() -> None:
    model = KineticModelBuilder().add_variable("v1", 1.0).add_parameter("k1", 1.0)
    with pytest.raises(NameError):
        model.add_reaction(
            "v1",
            fn=fns.mass_action_1s,
...
def test_add_derived_then_parameter_same_name() -> None:
    model = KineticModelBuilder().add_derived("d1", fns.constant, args=["x"])
    with pytest.raises(NameError):
