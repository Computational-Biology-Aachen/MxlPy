...
def test_generate_model_code_jax() -> None:
    assert meta.generate_model_code_jax(model_1rdo()).full().split("\n") == [
        "import jax",
        ...
        "    v1 = 1.0",
        "    v2 = 2.0",
        "    return (jnp.array([v1, v2]), jnp.array([]))",
    ]
...
def test_generate_model_code_cpp() -> None:
    assert meta.generate_model_code_cpp(model_1rdo()).full().split("\n") == [
        ...
        "}",
        "",
        "ignored fluxes(double time, const std::array<double, 2>& variables) {",
        "    const auto [v1, v2] = variables;",
        "    double p1 = 1.0;",
        "    double r1 = p1*v1;",
        "    double r2 = p1*v2;",
        ...
        "}",
        "",
        "ignored nv(const std::array<double, 2>& fluxes) {",
...
def test_generate_model_code_rs() -> None:
    assert meta.generate_model_code_rs(model_1rdo()).full().split("\n") == [
        ...
        "}",
        "",
        "fn fluxes(time: f64, variables: &[f64; 2]) -> ignored {",
        "    let [v1, v2] = *variables;",
        "    let p1: f64 = 1.0;",
        "    let r1: f64 = p1*v1;",
        "    let r2: f64 = p1*v2;",
