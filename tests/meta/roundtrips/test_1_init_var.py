from mxlpy import InitialAssignment, KineticModelBuilder, meta
...
def model_1_init_var() -> KineticModelBuilder:
    return (
        KineticModelBuilder()
        .add_variable(
            "v1",
...
def test_generate_model_code_jax() -> None:
    assert meta.generate_model_code_jax(model_1_init_var()).full().split("\n") == [
        ...
        "",
        "def model(time: jax.Array, variables: jax.Array, args: jax.Array) -> jax.Array:",
        "    (v1,) = variables",
        "",
        "    dv1dt = 0",
        "    return jnp.array([dv1dt])",
        "",
        "def derived(time: jax.Array, variables: jax.Array, args: jax.Array) -> jax.Array:",
        "    (v1,) = variables",
        "",
        "    p1 = 1.0",
        "    return jnp.array([])",
        "",
        "def fluxes(time: jax.Array, variables: jax.Array, args: jax.Array) -> jax.Array:",
        "    (v1,) = variables",
        "",
        "    return jnp.array([])",
        "",
        ...
        "    return jnp.array([dv1dt])",
        "",
        "def inits(time: jax.Array, variables: jax.Array, args: jax.Array) -> tuple[jax.Array, jax.Array]:",
        "    p1 = 1.0",
        "    v1 = 2.0*p1",
