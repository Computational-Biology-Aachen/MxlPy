...
def S5(k4: float, S2: float) -> float:
    return S2 * k4
...
def S4_conc(compartment1: float, S4: float) -> float:
    return S4 / compartment1


def S5_conc(S5: float, compartment1: float) -> float:
    return S5 / compartment1
...
def reaction2(S2_conc: float, S3_conc: float, k2: float) -> float:
    return k2 * (S2_conc - S3_conc)
...
def reaction3(compartment: float, S4_conc: float, S3_conc: float, k3: float) -> float:
    return S3_conc * S4_conc * compartment * k3


def reaction3_stoich_S3(compartment1: float) -> float:
    return -1.0 * compartment1


def reaction3_stoich_S4(compartment1: float) -> float:
    return 1.0 * compartment1
...
def create_model() -> Model:
    return (
        ...
        .add_variable("S2", initial_value=1.0)
        .add_variable("S3", initial_value=0.0)
        .add_variable("S4", initial_value=0.1)
        .add_parameter("k1", value=0.75)
        .add_parameter("k2", value=7.5)
        .add_parameter("k3", value=0.75)
        .add_parameter("k4", value=0.5)
        ...
        .add_derived(
            "S4_conc",
            fn=S4_conc,
            args=["compartment1", "S4"],
        )
        .add_derived(
            "S5_conc",
            fn=S5_conc,
            args=["S5", "compartment1"],
        )
        .add_reaction(
            "reaction1",
            fn=reaction1,
        ...
        .add_reaction(
            "reaction3",
            fn=reaction3,
            args=["compartment", "S4_conc", "S3_conc", "k3"],
            stoichiometry={
                "S3": Derived(fn=reaction3_stoich_S3, args=["compartment1"]),
                "S4": Derived(fn=reaction3_stoich_S4, args=["compartment1"]),
            },
        )
    )
