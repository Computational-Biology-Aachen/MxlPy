...
def reaction1(time: float, S1_conc: float, reaction1_k2: float) -> float:
    return S1_conc * reaction1_k2 * time
...
def reaction2(time: float, S2_conc: float, k2: float) -> float:
    return S2_conc * k2 * time
...
def reaction3(time: float, S3_conc: float, reaction3_k2: float) -> float:
    return S3_conc * reaction3_k2 * time
...
def create_model() -> Model:
    return (
        ...
        .add_parameter("reaction1_k2", value=0.7)
        .add_parameter("reaction3_k2", value=1.0)
        .add_derived(
            "S1_conc",
            fn=S1_conc,
            args=["C", "S1"],
        ...
        .add_reaction(
            "reaction1",
            fn=reaction1,
            args=["time", "S1_conc", "reaction1_k2"],
            stoichiometry={
        ...
        .add_reaction(
            "reaction3",
            fn=reaction3,
            args=["time", "S3_conc", "reaction3_k2"],
            stoichiometry={
