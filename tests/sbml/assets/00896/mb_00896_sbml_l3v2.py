...
def reaction2(time: float, reaction2_k1: float, S3_conc: float) -> float:
    return S3_conc * reaction2_k1 * time
...
def reaction3(reaction3_k1: float, time: float, S3_conc: float) -> float:
    return S3_conc * reaction3_k1 * time
...
def reaction3_stoich_S1(C: float) -> float:
    return 1.0 * C
...
def create_model() -> Model:
    return (
        ...
        .add_parameter("reaction2_k1", value=0.9)
        .add_parameter("reaction3_k1", value=0.7)
        .add_derived(
            "S1_conc",
            fn=S1_conc,
            args=["C", "S1"],
        ...
        .add_reaction(
            "reaction2",
            fn=reaction2,
            args=["time", "reaction2_k1", "S3_conc"],
            stoichiometry={
        ...
        .add_reaction(
            "reaction3",
            fn=reaction3,
            args=["reaction3_k1", "time", "S3_conc"],
            stoichiometry={
                "S3": Derived(fn=reaction3_stoich_S3, args=["C"]),
                "S1": Derived(fn=reaction3_stoich_S1, args=["C"]),
