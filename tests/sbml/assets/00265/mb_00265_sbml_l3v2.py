...
def create_model() -> Model:
    return (
        ...
        .add_variable("S2", initial_value=0.005)
        .add_variable("S3", initial_value=0.01)
        .add_parameter("k1", value=160.0)
        .add_parameter("k2", value=0.7)
        .add_parameter("compartment", value=math.nan)
        .add_reaction(
            "reaction1",
            fn=reaction1,
            args=["S1", "k1", "S2"],
            stoichiometry={"S1": -2.0, "S2": -1.0, "S3": 1.0},
        )
        .add_reaction(
            "reaction2",
            fn=reaction2,
            args=["S3", "k2"],
            stoichiometry={"S3": -1.0, "S1": 2.0, "S2": 1.0},
        )
    )
