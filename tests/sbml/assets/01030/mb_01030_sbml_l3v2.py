...
def reaction1(
    S2_conc: float, S1_conc: float, reaction1_kr: float, reaction1_kf: float
) -> float:
...
def reaction2(S4_conc: float, S3_conc: float, kf: float, kr: float) -> float:
    return -S3_conc * kf + S4_conc * kr
...
def create_model() -> Model:
    return (
        ...
        .add_reaction(
            "reaction1",
            fn=reaction1,
            args=["S2_conc", "S1_conc", "reaction1_kr", "reaction1_kf"],
            stoichiometry={
