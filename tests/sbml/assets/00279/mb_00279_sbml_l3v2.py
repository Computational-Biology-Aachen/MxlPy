...
def reaction2(S2_conc: float, p1: float, p2: float) -> float:
    return (
        (p1)
        if ((S2_conc >= 1) or (S2_conc >= 2) or (S2_conc > 4))
        and ((S2_conc >= 1) or (S2_conc <= 4) or (S2_conc < 2))
        and ((S2_conc >= 2) or (S2_conc <= 4) or (S2_conc < 1))
        and ((S2_conc > 4) or (S2_conc < 1) or (S2_conc < 2))
        else (p2)
    )
...
def create_model() -> Model:
    return (
        Model()
        .add_variable("S1", initial_value=5.0)
        .add_variable("S2", initial_value=0.0)
        .add_variable("S3", initial_value=4.0)
