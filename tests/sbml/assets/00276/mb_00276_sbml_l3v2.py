...
def reaction2(S2_conc: float, S1_conc: float, p1: float, p2: float) -> float:
    return (p1) if (S2_conc > 4) and (S1_conc < 5) and (S2_conc < 8) else (p2)
...
def create_model() -> Model:
    return (
        Model()
        .add_variable("S1", initial_value=10.0)
        .add_variable("S2", initial_value=0.0)
        .add_variable("S3", initial_value=10.0)
