...
def reaction1(time: float, S1_conc: float, k1: float) -> float:
    return S1_conc * k1 * time
...
def create_model() -> Model:
    return (
        ...
        .add_reaction(
            ...
            stoichiometry={"S2": Derived(fn=reaction2_stoich_S2, args=["C"])},
        )
        .add_reaction(
            "reaction3",
            fn=reaction3,
            args=["k3", "time", "S3_conc"],
            stoichiometry={"S4": Derived(fn=reaction3_stoich_S4, args=["C"])},
        )
    )
